...
wait_for_simulink = 0 # Wait for Simulink node to start before activating
...
# Random waypoints and time intervals (from MATLAB script)
waypoints = [
                [0, 0, -10, 0],
                [4.0736, 1.8116, -10, 0],
...
# Send next waypoint after waypoint_time[index] passed.
waypoints_time = [
                0,
                9,
                12,
    ...
    if auto_offboard:
        ...
        if publish_to_mavros: 
            if wait_for_simulink:
                print("Waiting for Simulink node...")
