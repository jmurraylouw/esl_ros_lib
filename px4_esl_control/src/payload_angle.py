...
class Sub: 
    ...
    def acc_sp_cb(self, msg):  # Callback function for acceleration setpoint subscriber
        self.acc_sp = msg.acceleration_or_force
...
def run(argv):
    ...
    column_headings = ['current_time',    'velocity.x', 'velocity.y', 'velocity.z',    'acc_sp.x', 'acc_sp.y', 'acc_sp.y',    'payload_angles.x', 'payload_angles.y']
    ...
    while not rospy.is_shutdown():
        ...
        new_row = [current_time,    velocity.x, velocity.y, velocity.z,    acc_sp.x, acc_sp.y, acc_sp.y,    payload_angles.x, payload_angles.y] # New row to append to log file
